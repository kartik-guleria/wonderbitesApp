export default {
  languageSelector: 'Select Your Language',

  //Common Screens
  addAddress: 'Add Address',
  // AboutUs Screen
  AboutUs: 'About Us',
  TermsAndConditions: 'Terms And Conditions',
  ChangeEmail: 'Change Email',
  DeleteAccount: 'Delete Account',
  Customer: 'Customer',
  CustomerDesc:
    'Customer or "You" or "Your" refers to you, as a customer of the Services. A customer is someone who accesses or uses the Services for the purpose of sharing, displaying, hosting, publishing, transacting, or uploading information or views or pictures and includes other persons jointly participating in using the Services including without limitation a user having access to restaurant business page to manage claimed business listings or otherwise.',
  Content: 'Content',
  ContentDesc:
    '"Content" will include (but is not limited to) reviews, images,photos, audio, video, location data, nearby places, and all other forms of information or data. "Your content" or "Customer Content" means content that you upload, share or transmit to, through or in connection with the Services, such as likes, ratings, reviews, images, photos, messages, profile information, and any other materials that you publicly display or displayed in your account profile.',
  Eligibility: 'Eligibility to use the services',
  EligibilityDesc:
    'You hereby represent and warrant that you are at least eighteen (18) years of age or above and are fully able and competent to understand and agree the terms, conditions, obligations, affirmations, representations, and warranties set forth in these Terms. Compliance with Laws. You are in compliance with all laws and regulations in the country in which you live when you access and use the Services You agree to use the Services only in compliance with these Terms and applicable law, and in a manner that does not violate our legal rights or those of any third parties.',

  // AccountSettings Screen
  AccountSettings: 'Account Settings',
  changeEmail: 'Change Email',
  deleteAccount: 'Delete Account',
  //Flatlist items

  //ChangeEmail Screen
  ToChange: 'To change your email go to',

  //Home Screen
  //Flatlist Text items
  delivery: 'languagechanges',

  //RegisterScreen
  PersonalDetails: 'Personal Details',
  tellUs: 'Tell us more about yourself',
  FirstName: 'First Name',
  LastName: 'Last Name',
  Email: 'Email',
  Register: 'Register',

  //Profile Screen
  Profile: 'Profile',
  //Flatlist Text items
  personal: 'Personal Details',
  myWonderpoints: 'My wonderpoints',
  settings: 'Settings',
  notifications: 'Notifications',
  orderHistory: 'Order History',
  mySubscriptions: 'My subscriptions',
  wishlist: 'Wishlist',
  myBookings: 'My bookings',
  chatSupport: 'Chat Support',
  about: 'About',
  SendFeedback: 'Send Feedback',
  LogOut: 'Log out',
  //Rate Us Screen
  RateUs: 'Rate Us',
  YourOpinion: 'Your opinion matters to us!',
  OpinionDesc:
    'We work super hard to make Wonderbites better for you, and we would love to know:',
  HowWould: 'How would you rate our app?',
  NoThanks: 'No Thanks',

  //ResetPassword Screen
  ResetPassword: 'Reset Password',
  ResetPasswordDesc:
    'Enter the email associated with your account and we will send an email with instructions to reset your password',
  ResetPasswordEmail: 'Email',
  SendInstructions: 'Send Instructions',

  //Review Screen
  Reviews: 'Reviews',
  //Flatlist Text Items

  //SelectLanguage Screen
  Language: 'Language',
  english: 'English',
  albanian: 'Albanian',
  french: 'French',
  italian: 'Italian',
  greek: 'Greek',
  macedonian: 'Macedonian',

  //Settings Screen
  Settings: 'Settings',
  appPermission: 'App Permissions',
  openPhone: 'Open your phone settings',
  accountSettings: 'Account Settings',
  changeEmailOrDelete: 'Change your email or delete your account',
  paymentDetails: 'Payment Details',
  editPayment: 'Edit your payment details',
  myAddresses: 'My Addresses',
  editAddress: 'Edit your address details',
  language: 'Language',
  english: 'English',
  appVersion: 'App Version',
  //Flatlist Text Items

  //SideMenuScreen
  SignIn: 'Sign in',
  ToWB: 'to wonderbites',
  SignInJoin: 'Sign in or join',
  callUs: 'Call Us',
  terms: 'Terms',
  privacyPolicy: 'Privacy policy',
  contentPolicy: 'Content policy',
  english: 'English',
  followUs: 'Follow Us',

  //SignIn Screen
  LogInSign: 'Log in',
  Email: 'Email',
  Password: 'Password',
  Forgot: 'forgot your password?',
  LogIn: 'Log In',
  DontHave: 'Don’t have an account? ',
  SignUp: 'SignUp',

  //SignInToJoin Screen
  skip: 'Skip',
  Choose: 'Choose an option to ',
  logIn: 'Log In',
  SendOtp: 'Send OTP',
  Or: 'OR',
  ContinueWithEmail: 'Continue with Email',
  facebook: 'Facebook',
  google: 'Google',

  //SignUp Screen
  SignUp: 'Sign up',
  FirstName: 'First Name',
  LastName: 'Last Name',
  Email: 'Email',
  ContactNumber: 'Contact Number',
  Password: 'Password',
  ConfirmPassword: 'Confirm Password',
  IAgree: 'I agree with our ',
  Terms: 'Terms',
  And: 'And ',
  Conditions: 'Conditions',
  Register: 'Register',
  AlreadyHave: ' Already have an account? ',
  Login: 'Log in',

  //TermsCondition Screen
  ContentPolicy: 'Content Policy',
  Update: 'Update',
  HowWeUse: 'How We Use Your Personal Information',

  //VerificationCode Screen
  VerificationCode: 'Verification Code',
  WeSentCode: 'We sent you a code to your phone number',
  SentTo: 'Sent To',
  didNotReceive: 'I didn’t receive a code!',
  resend: 'Resend',
  IfInvalid: 'Verification code is invalid',
  TryAgain: 'Try Again',
  Submit: 'Submit',

  //Welcome Screen
  signInJoin: 'Sign in or join',
  orderNow: 'Order now',
};
